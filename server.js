...
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || ""; // Imposta la chiave in Environment su Render

app.use(express.json());

// Database SQLite in memoria o su file
...
// Funzione helper per chiamare le API di Gemini
function callGeminiAPI(prompt) {
return new Promise((resolve, reject) => {
if (!GEMINI_API_KEY) {
return reject(new Error("GEMINI_API_KEY non configurata. Inseriscila nelle Environment Variables di Render."));
}
...
reject(new Error("Risposta API Gemini non valida: " + body));
}
} catch (e) {
reject(e);
}
...
// Generazione Esercitazione con Gemini AI
...
// Ottieni storico e analisi errori per Sezione Apprendimento
...
<button class="tab-btn" onclick="switchAuthTab('register')">Nuovo Profilo</button>
<button class="tab-btn" onclick="switchAuthTab('recover')">Recupero PIN</button>
</div>
...
<div class="tabs">
<button class="tab-btn active" onclick="switchAppTab('exercise')">Esercitazione Unità</button>
<button class="tab-btn" onclick="switchAppTab('learning')">Sezione Apprendimento (Errori)</button>
</div>
...
if(tab === 'login') {
document.querySelectorAll('#auth-section .tab-btn')[0].classList.add('active');
document.getElementById('login-form').classList.remove('hidden');
} else if(tab === 'register') {
document.querySelectorAll('#auth-section .tab-btn')[1].classList.add('active');
document.getElementById('register-form').classList.remove('hidden');
} else {
document.querySelectorAll('#auth-section .tab-btn')[2].classList.add('active');
document.getElementById('recover-form').classList.remove('hidden');
}
}

function switchAppTab(tab) {
document.querySelectorAll('#app-section .tabs .tab-btn').forEach(b => b.classList.remove('active'));
document.getElementById('tab-exercise').classList.add('hidden');
document.getElementById('tab-learning').classList.add('hidden');

if(tab === 'exercise') {
document.querySelectorAll('#app-section .tabs .tab-btn')[0].classList.add('active');
document.getElementById('tab-exercise').classList.remove('hidden');
} else {
document.querySelectorAll('#app-section .tabs .tab-btn')[1].classList.add('active');
...
// Spiegazione Errori/Apprendimento
...
// Salva nel database per la Sezione Apprendimento
...
\${!isCorrect ? \`<p style="font-size:0.85rem; color:#15803d; margin-top:4px;"><strong>Corretta:</strong> \${q.options[q.correct]}</p>
